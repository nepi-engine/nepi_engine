...
# image connect pattern (ImageStatus status + Image data + cv2 image dict), so
# ConnectBaseImageIF reuses it directly and serves as the shared parent for the
# image connect subclasses below.


class ConnectBaseImageIF(ConnectDataIF):
    """Shared parent connect class for the image data-interface family.

    Mirrors data_if.BaseImageIF. Inherits the complete image connect
    implementation (status/ready/connection handling, cv2 image dict accessors,
    save/config methods, and private callbacks) from ConnectDataIF.
    """
    pass
...
# this connect class subclasses ConnectNodeIF directly and mirrors the
# ConnectDataIF image-handling pattern with a DepthMapStatus status type.
...
class ConnectDepthMapIF(ConnectNodeIF):
    ...
    def get_namespace(self):
        """Return the fully-resolved ROS namespace for the connected depth map source.
    ...
    def check_connection(self):
        """Check whether the depth map source is currently connected.
    ...
    def wait_for_connection(self, timeout = float('inf') ):
        """Block until the depth map source is connected or the timeout expires.
    ...
    def check_status_connection(self):
        """Check whether the status topic from the depth map source is currently connected.
    ...
    def wait_for_status_connection(self, timeout = float('inf') ):
        """Block until the depth map source status topic is connected or the timeout expires.
    ...
    def get_status_dict(self):
        """Return the latest depth map source status as a dictionary.

        Returns:
            dict: A dictionary representation of the most recent DepthMapStatus message,
                or None if no status has been received yet.
        """
        status_dict = None
        if self.status_msg is not None:
    ...
    def get_status_msg(self):
        """Return the latest depth map source status as a msg.

        Returns:
            dict: A msg representation of the most recent DepthMapStatus message,
                or None if no status has been received yet.
        """
        return self.status_msg

    def get_data_topic(self):
        """Return the depth map data topic of the connected data source.

        Returns:
            str: The selected data source namespace, which is also the Image topic
                the source publishes the depth map on, or 'None' if no source is selected.
        """
        return self.selected_topic
    ...
    def set_get_data(self, state):
        """Set the flag requesting capture of the next available depth map frame.
...
# connect class subclasses ConnectNodeIF directly and mirrors the ConnectDataIF
# pattern with a PointcloudStatus status type and an Open3D point cloud dict.
...
class ConnectPointcloudIF(ConnectNodeIF):
    ...
    def get_namespace(self):
        """Return the fully-resolved ROS namespace for the connected pointcloud source.
    ...
    def check_connection(self):
        """Check whether the pointcloud source is currently connected.
    ...
    def wait_for_connection(self, timeout = float('inf') ):
        """Block until the pointcloud source is connected or the timeout expires.
    ...
    def check_status_connection(self):
        """Check whether the status topic from the pointcloud source is currently connected.
    ...
    def wait_for_status_connection(self, timeout = float('inf') ):
        """Block until the pointcloud source status topic is connected or the timeout expires.
    ...
    def get_status_dict(self):
        """Return the latest pointcloud source status as a dictionary.

        Returns:
            dict: A dictionary representation of the most recent PointcloudStatus message,
                or None if no status has been received yet.
        """
        status_dict = None
        if self.status_msg is not None:
    ...
    def get_status_msg(self):
        """Return the latest pointcloud source status as a msg.

        Returns:
            dict: A msg representation of the most recent PointcloudStatus message,
                or None if no status has been received yet.
        """
        return self.status_msg

    def get_data_topic(self):
        """Return the pointcloud data topic of the connected data source.

        Returns:
            str: The selected data source namespace, which is also the PointCloud2 topic
                the source publishes on, or 'None' if no source is selected.
        """
        return self.selected_topic
    ...
    def set_get_data(self, state):
        """Set the flag requesting capture of the next available pointcloud frame.
