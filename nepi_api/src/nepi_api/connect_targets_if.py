...
class ConnectTargetsIF:
  

    DEFAULT_TARGETS = ['boat','bottle','cup','person']

    FILTER_OPTIONS = nepi_targets.TARGET_FILTER_OPTIONS
    DEFAULT_FILTER = 'LARGEST'
    ...
    targets_ordered_list = DEFAULT_TARGETS
    targets_max_update_rate = MAX_UPDATE_RATE
    targets_filter = DEFAULT_FILTER
    targets_min_error_deg = MIN_ERROR_DEG
    targets_source_topic = DEFAULT_SOURCE
    targets_source_connected_namespace = "None"
    targets_source_connected = False
    targets_source_connecting = False

    targets_exit_process = EXIT_FUNCTION
    targets_source_namespace = 'None'
    targets_last_namespace = 'None'
    targets_if = None
    targets_dict = None
    ...
    def __init__(self,
        ...
        self.targets_source_namespace = os.path.join(self.base_namespace,self.targets_source_topic)
       ...
       # Configure PTX Capabilities
        ...
        if self.has_auto_pan:
            ...
            nepi_sdk.start_timer_process(self.targets_max_update_rate, self.trackPanProcess)
        ...
        if self.has_auto_tilt:
            ...
            nepi_sdk.start_timer_process(self.targets_max_update_rate, self.trackTiltProcess)
    ...
    def autoTiltProcess(self,timer):
        ...
        if self.auto_tilt_enabled == False:
            ...
            if self.targets_tilt_enabled == True:
                self.targets_tilt_enabled = False
    ...
    def trackPanProcess(self,timer):
        if self.targets_pan_enabled == True and self.targets_source_connected == True:
            pan_cur = self.current_position[0]
            targets_dict = copy.deepcopy(self.targets_dict)
            if targets_dict is not None:
                pan_error = targets_dict['azimuth_deg']
                ...
                if abs(avg_error) > self.targets_min_error_deg:
    ...
    def trackTiltProcess(self,timer):
        if self.targets_tilt_enabled == True and self.targets_source_connected == True:
            tilt_cur = self.current_position[1]
            targets_dict = copy.deepcopy(self.targets_dict)
            if targets_dict is not None:
                tilt_error = targets_dict['elevation_deg']
    ...
    def setAutoPan(self,enabled):
        self.targets_pan_enabled = False
    ...
    def setTrackPan(self,enabled):
        ...
        self.nav_lock_pan_enabled = False
        self.targets_pan_enabled = enabled
        self.publish_status()
        self.node_if.set_param('targets_pan_enabled', self.targets_pan_enabled)
    ...
    def setTrackTilt(self,enabled):
        ...
        self.nav_lock_tilt_enabled = False
        self.targets_tilt_enabled = enabled
        self.publish_status()
        self.node_if.set_param('targets_tilt_enabled', self.targets_tilt_enabled)
    ...
    def connectTrackSourceCb(self,timer):
        targets_namespace = copy.deepcopy(self.targets_source_namespace)
        if targets_namespace != self.targets_source_connected_namespace  and self.targets_source_connecting == False:
            if self.targets_if is not None:
                self.targets_if = None
                nepi_sdk.sleep(1)
            self.targets_source_connected = False
            #self.msg_if.pub_warn("set_targets_source connected False")
            if nepi_sdk.find_topic(targets_namespace) != "": 

                self.targets_source_connecting = True                 
                self.targets_if = nepi_sdk.create_subscriber(targets_namespace, Targets, self.targetsCb, queue_size = 1, callback_args= (targets_namespace), log_name_list = [])
    ...
    def targetsCb(self,msg, args):
        ...
        self.targets_source_connected = True
        #self.msg_if.pub_warn("set_targets_source connected True")
        self.targets_source_connected_namespace = self.targets_source_namespace
        self.targets_source_connecting == False
        self.last_targets_msg = nepi_utils.get_time()
        targets_dict_list = []
        ...
        if len(targets_dict_list) > 0:
            ...
            targets_dict = nepi_targets.filter_targets_list(targets_dict_list,self.targets_ordered_list,self.targets_filter)
        #self.msg_if.pub_warn("Got track dict " + str(targets_dict))
