...
class DepthMapImgPub:
    ...
    def __init__(self):
        ...
        self.msg_if.pub_warn("Starting with node namespace: " + str(self.node_namespace))            

        self.data_product = nepi_sdk.get_param("~data_product",self.IMG_DATA_PRODUCT)
        self.data_products = [self.data_product]
        self.msg_if.pub_warn("Starting with Image Data Product: " + str(self.data_product))
        
        backup_dm_namespace = "None" #self.node_namespace.replace("_img_pub","/depth_map")
        self.dm_namespace = nepi_sdk.get_param("~dm_namespace",backup_dm_namespace)
        self.msg_if.pub_warn("Starting with data namespace: " + str(self.dm_namespace))  

        self.bs_namespace = os.path.dirname(self.dm_namespace)
        self.msg_if.pub_warn("Starting with source node namespace: " + str(self.bs_namespace))  
    ...
    def updaterCb(self,timer):
        ...
        if connected == False and self.img_info_dict is None:
            if self.dm_namespace is not "None":
                success = self.subscribeImgTopic()     

        nepi_sdk.start_timer_process((.5), self.updaterCb, oneshot = True)


    def subscribeImgTopic(self):
        img_sub_dict = {
                    'namespace': self.dm_namespace,
                    'msg': Image,
                    'topic': '',
                    ...
                    'callback_args': (self.dm_namespace)
                }
        ...
        if img_info_dict is not None:
            if self.img_ifs_dict:
                self.msg_if.pub_info('Subsribing to image topic: ' + self.dm_namespace)  
                ...
                self.msg_if.pub_warn('Registered : ' + self.dm_namespace +  ' ' + str(self.img_ifs_dict))
                # Set back to active
                self.img_info_dict['active'] = True
        else:
                
            # Create register new image topic
            self.msg_if.pub_warn('Registering to topic: ' + self.dm_namespace)
            bs_namespace = os.path.dirname(self.dm_namespace)
            pub_namespace = nepi_sdk.create_namespace(bs_namespace,self.data_product)
            self.msg_if.pub_warn('Publishing to namespace: ' + pub_namespace)
            ...
            self.msg_if.pub_info('Subsribing to topic: ' + self.dm_namespace)
            ...
            self.img_ifs_lock.release()
            self.msg_if.pub_warn('Registered : ' + self.dm_namespace)
    ...
    def unsubscribeImgTopic(self):
        self.msg_if.pub_warn('Unregistering image topic: ' + self.dm_namespace)
        self.img_ifs_lock.acquire()
    ...
    def depthMapCb(self, depth_map_msg, args):     
        ...
        max_rate = copy.deepcopy(self.max_pub_rate)
        if max_rate > .01:
