...
API_FOLDER=$(cd -P "$(dirname -- "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)
API_NAME=$(basename "$API_FOLDER")

###############################################
# Deploy Nepi API Scripts
###############################################
SOURCE_PATH=$API_FOLDER
DEST_PATH="/mnt/nepi_storage/nepi_src/nepi_engine_ws/lib/nepi_api"
...
echo ""
echo "Syncing ${API_NAME} scripts from ${SOURCE_PATH} to NEPI Build Repo at:" 
...
echo ""
echo "Syncing ${API_NAME} scripts from ${SOURCE_PATH} to NEPI Live Folders at:" 
echo "Destination Path ${DEST_PATH}"
echo ""
