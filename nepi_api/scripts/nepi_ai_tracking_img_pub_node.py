...
from nepi_interfaces.msg import AiDetectorStatus, AiTrackingStatus
...
class AiDetectorImgPub:
    ...
    tracking_sub_names = ['bounding_boxes']
    ...
    def __init__(self):
        ...
        backup_tracking_namespace = self.node_namespace.replace("_img_pub","")
        self.tracking_namespace = nepi_sdk.get_param(self.node_namespace + "/tracking_namespace",backup_tracking_namespace)
        self.msg_if.pub_warn("Starting with Detector Namespace: " + str(self.tracking_namespace))
        ...
        self.status_msg = AiTrackingStatus()
        ...
        self.img_tracking_namespaces = []
        self.img_tracking_states = []
        ...
        '''
                ...
                'init_configs': True,
                'namespace':  self.tracking_namespace,
        }
        '''
        ...
        self.save_data_if = SaveDataIF(data_products = self.data_products, pub_status = False, factory_rate_dict = factory_data_rates, namespace = self.tracking_namespace,
                        msg_if = self.msg_if
                        )
    ...
    def updaterCb(self,timer):
        ...
        for img_topic in self.imgs_info_dict.keys():
            last_time = self.imgs_info_dict[img_topic]['last_tracking_time']
            check_time = current_time - last_time
            '''
            if check_time > self.clear_tracking_time or self.enabled == False or self.state_str_msg != 'Detecting':
                try:
                    self.imgs_info_dict[img_topic]['tracking_dict_list'] = None
                except:
                    pass
            '''
    ...
    def subscribeImgTopic(self,img_topic):
        ...
        tracking_name = os.path.basename(self.tracking_namespace)
        #self.msg_if.pub_warn('Creating namespace for image name: ' + img_source_topic)

        pub_namespace = img_source_topic #os.path.join(os.path.dirname(img_topic),tracking_name)
        ...
        img_info_dict['last_tracking_time'] = 0
        img_info_dict['tracking_dict_list'] = []   
        ...
        if img_topic in self.imgs_info_dict.keys():
            if self.imgs_info_dict[img_topic]['active'] == True:
                ...
                if img_topic in self.imgs_info_dict.keys():
                    ...
                    self.imgs_info_dict[img_topic]['status_dict'] = None
                    ...
                    self.imgs_info_dict[img_topic]['last_img'] = None
    ...
    def imageStatusCb(self, status_msg, args):   
            ...
            if img_topic in self.imgs_info_dict.keys():
                ...
                if self.imgs_info_dict[img_topic]['status_dict'] is None:
                ...
                self.imgs_info_dict[img_topic]['status_dict'] = status_dict
            self.imgs_info_lock.release()                
                ...
                if img_topic in self.imgs_info_dict.keys() and img_topic in sel_imgs and max_rate > .01:
                    ...
                    if self.enabled == True and self.state_str_msg == 'Detecting':
                        ...
                        if timer > delay_time: 
                            ...
                            # Request new img
                            tracking_dict_list = copy.deepcopy(self.imgs_info_dict[img_topic]['tracking_dict_list'])
                            #self.msg_if.pub_info("Got Detection List: " + str(tracking_dict_list))
                            if tracking_dict_list == None:
                                tracking_dict_list = []
                            ...
                            if self.use_last_image == False:
                                ...
                                use_cv2_img = copy.deepcopy(self.imgs_info_dict[img_topic]['last_img'])
                            ...
                            if self.use_last_image == True:
                                # process image for next time
                                self.imgs_info_dict[img_topic]['last_img'] = nepi_img.rosimg_to_cv2img(image_msg)

                           

    def processFileImg(self, img_file,tracking_dict_list,loc_dict_list):   
        img_topic = 'img_file'      
        max_rate = copy.deepcopy(self.max_rate)
        if max_rate > .01:
            if self.enabled == True and self.state_str_msg == 'Detecting':
    ...
    def publishImgData(self, img_topic, cv2_img, encoding = "bgr8", timestamp = None, add_overlay_list = []):
            ...
            if self.pub_image_enabled:
                ...
                if img_topic in self.imgs_info_dict.keys():
                    ...
                    status_dict = copy.deepcopy(self.imgs_info_dict[img_topic]['status_dict'])
    ...
    def apply_detection_overlay(self,img_topic, detect_dict_list, loc_dict_list, cv2_img):
        cv2_tracking_img = copy.deepcopy(cv2_img)
        ...
        for i, detect_dict in enumerate(detect_dict_list):
            ...
            success = False
            try:
                cv2.rectangle(cv2_tracking_img, bot_left_box, top_right_box, class_color, thickness=line_thickness)
                success = True
            ...
            if success == True:
                ...
                if len(overlay_text) > 0:
                    ...
                    try:
                        cv2.rectangle(cv2_tracking_img, bot_left_box, top_right_box, box_color , -1)
                        cv2.putText(cv2_tracking_img,text2overlay, 
                            bot_left_text, 
                            font, 
                            fontScale,
                            fontColor,
