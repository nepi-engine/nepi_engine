...
def rbx_initialize(self, rbx_namespace):
  ...
  nepi_sdk.log_msg_info("NEPI_RBX: Found rbx namespace: " + NEPI_RBX_NAMESPACE)
  nepi_sdk.log_msg_info("NEPI_RBX: Found rbx status topic: " + rbx_topic)
  ...
  rbx_caps_service = nepi_sdk.connect_service(NEPI_RBX_CAPABILITIES_TOPIC, RBXCapabilitiesQuery)
  time.sleep(1)
  rbx_caps = rbx_caps_service()
  nepi_sdk.log_msg_info(rbx_caps)
  ...
  # nepi_sdk.log_msg_info some results
  nepi_sdk.log_msg_info("NEPI_RBX: RBX State Options: ")
  for state in self.rbx_cap_states:
     nepi_sdk.log_msg_info("NEPI_RBX:  - " + state)
  nepi_sdk.log_msg_info("NEPI_RBX: RBX Mode Options: ")
  for mode in self.rbx_cap_modes:
     nepi_sdk.log_msg_info("NEPI_RBX:  - " + mode)
  nepi_sdk.log_msg_info("NEPI_RBX: RBX Setup Action Options: ")
  for action in self.rbx_cap_setup_actions:
     nepi_sdk.log_msg_info("NEPI_RBX:  - " + action)
  nepi_sdk.log_msg_info("NEPI_RBX: RBX Go Action Options: ")
  ...
  nepi_sdk.log_msg_info("DRONE_INSPECT: Waiting for topic: " + self.NEPI_RBX_INFO_TOPIC)
  nepi_sdk.wait_for_topic(self.NEPI_RBX_INFO_TOPIC)
  rbx_info_pub = nepi_sdk.create_publisher(NEPI_RBX_NAMESPACE + 'publish_info', Empty, queue_size=1)
  nepi_sdk.log_msg_info("DRONE_INSPECT: Starting rbx info scubscriber callback")
  nepi_sdk.create_subscriber(self.NEPI_RBX_INFO_TOPIC,RBXInfo, self.rbx_info_callback, queue_size=None)
  while self.rbx_info is None and not nepi_sdk.is_shutdown():
    nepi_sdk.log_msg_info("DRONE_INSPECT: Waiting for current rbx info to publish")
    time.sleep(1)
    rbx_info_pub.publish(Empty())
  info_str = str(self.rbx_info)
  nepi_sdk.log_msg_info("DRONE_INSPECT: " + info_str)

  ## Setup Status Update Callback
  self.NEPI_RBX_STATUS_TOPIC = NEPI_RBX_NAMESPACE + "status" # RBX Status Message
  nepi_sdk.log_msg_info("DRONE_INSPECT: Waiting for topic: " + self.NEPI_RBX_STATUS_TOPIC)
  nepi_sdk.wait_for_topic(self.NEPI_RBX_STATUS_TOPIC)
  rbx_status_pub = nepi_sdk.create_publisher(NEPI_RBX_NAMESPACE + 'publish_status', Empty, queue_size=1)
  nepi_sdk.log_msg_info("DRONE_INSPECT: Starting rbx status scubscriber callback")
  nepi_sdk.create_subscriber(self.NEPI_RBX_STATUS_TOPIC, RBXStatus, self.rbx_status_callback, queue_size=None)
  while self.rbx_status is None and not nepi_sdk.is_shutdown():
    nepi_sdk.log_msg_info("DRONE_INSPECT: Waiting for current rbx status to publish")
    time.sleep(0.1)
 ...
 # NEPI RBX Driver Control Topics
  ...
  self.rbx_set_state_pub = nepi_sdk.create_publisher(NEPI_RBX_SET_STATE_TOPIC, Int32, queue_size=1)
  self.rbx_set_mode_pub = nepi_sdk.create_publisher(NEPI_RBX_SET_MODE_TOPIC, Int32, queue_size=1)
  self.rbx_setup_action_pub = nepi_sdk.create_publisher(NEPI_RBX_SETUP_ACTION_TOPIC, Int32, queue_size=1)
  self.rbx_set_cmd_timeout_pub = nepi_sdk.create_publisher(NEPI_RBX_SET_CMD_TIMEOUT_TOPIC, UInt32, queue_size=1)
  self.rbx_set_home_pub = nepi_sdk.create_publisher(NEPI_RBX_SET_HOME_TOPIC, GeoPoint, queue_size=1)
  self.rbx_set_image_topic_pub = nepi_sdk.create_publisher(NEPI_RBX_SET_STATUS_IMAGE_TOPIC, String, queue_size=1)
  self.rbx_set_process_name_pub = nepi_sdk.create_publisher(NEPI_RBX_SET_PROCESS_NAME_TOPIC, String, queue_size=1)
  ...
  self.rbx_go_action_pub = nepi_sdk.create_publisher(NEPI_RBX_GO_ACTION_TOPIC, Int32, queue_size=1)
  self.rbx_go_home_pub = nepi_sdk.create_publisher(NEPI_RBX_GO_HOME_TOPIC, Empty, queue_size=1)
  self.rbx_go_stop_pub = nepi_sdk.create_publisher(NEPI_RBX_GO_STOP_TOPIC, Empty, queue_size=1)
  self.rbx_goto_pose_pub = nepi_sdk.create_publisher(NEPI_RBX_GOTO_POSE_TOPIC, RBXGotoPose, queue_size=1)
  self.rbx_goto_position_pub = nepi_sdk.create_publisher(NEPI_RBX_GOTO_POSITION_TOPIC,RBXGotoPosition, queue_size=1)
  self.rbx_goto_location_pub = nepi_sdk.create_publisher(NEPI_RBX_GOTO_LOCATION_TOPIC, RBXGotoLocation, queue_size=1)

  # Fake GPS Controls
  NEPI_RBX_FAKE_GPS_ENABLE_TOPIC = NEPI_RBX_NAMESPACE + "enable_fake_gps" 
  self.rbx_enable_fake_gps_pub = nepi_sdk.create_publisher(NEPI_RBX_FAKE_GPS_ENABLE_TOPIC, Bool, queue_size=1)

  nepi_sdk.log_msg_info("NEPI_RBX: RBX initialize process complete")
...
def get_capabilities(self,caps_topic):
  ...
  rbx_caps_service = nepi_sdk.connect_service(caps_topic, RBXCapabilitiesQuery)
  time.sleep(1)
  rbx_caps = rbx_caps_service()
  return rbx_caps
...
def get_navpose_capabilities(self,caps_navpose_topic):
  ...
  rbx_cap_navpose_service = nepi_sdk.connect_service(caps_navpose_topic, NPXCapabilitiesQuery)
  time.sleep(1)
...
def set_rbx_state(self,state_str,timeout_sec = 5):
  nepi_sdk.log_msg_info("NEPI_RBX: *******************************")  
  nepi_sdk.log_msg_info("NEPI_RBX: Set State Request Recieved: " + state_str)
  success = False
  ...
  for ind, state in enumerate(self.rbx_cap_states):
    if state == state_str:
      new_state_ind = ind
  if new_state_ind == -1:
    nepi_sdk.log_msg_info("NEPI_RBX: No matching state found")
  else:
    nepi_sdk.log_msg_info("NEPI_RBX: Setting state to: " + state_str)
    self.rbx_set_state_pub.publish(new_state_ind)
    timeout_timer = 0
    sleep_time_sec = 1
    while self.rbx_info.state != new_state_ind and timeout_timer < timeout_sec and not nepi_sdk.is_shutdown():
      nepi_sdk.log_msg_info("NEPI_RBX: Waiting for rbx state " + self.rbx_cap_states[new_state_ind] + " to set")
      nepi_sdk.log_msg_info("NEPI_RBX: Current rbx state is " + self.rbx_cap_states[self.rbx_info.state])
...
def set_rbx_mode(self,mode_str, timeout_sec = 5):
  nepi_sdk.log_msg_info("NEPI_RBX: *******************************")  
  nepi_sdk.log_msg_info("NEPI_RBX: Set Mode Request Recieved: " + mode_str)
  success = False
  self.rbx_set_cmd_timeout_pub.publish(timeout_sec)
  new_mode_ind = -1
  for ind, mode in enumerate(self.rbx_cap_modes):
    if mode == mode_str:
      new_mode_ind = ind
  if new_mode_ind == -1:
    nepi_sdk.log_msg_info("NEPI_RBX: No matching mode found")
  else:
    nepi_sdk.log_msg_info("NEPI_RBX: Setting mode to: " + mode_str)
    self.rbx_set_mode_pub.publish(new_mode_ind)
    timeout_timer = 0
    sleep_time_sec = 1
    while self.rbx_info.mode != new_mode_ind and timeout_timer < timeout_sec and not nepi_sdk.is_shutdown():
      nepi_sdk.log_msg_info("NEPI_RBX: Waiting for rbx mode " + self.rbx_cap_modes[new_mode_ind] + " to set")
      nepi_sdk.log_msg_info("NEPI_RBX: Current rbx mode is " + self.rbx_cap_modes[self.rbx_info.mode])
      time.sleep(sleep_time_sec)
      timeout_timer = timeout_timer + sleep_time_sec
...
def setup_rbx_action(self,action_str,timeout_sec = 10):
  nepi_sdk.log_msg_info("NEPI_RBX: *******************************")  
  nepi_sdk.log_msg_info("NEPI_RBX: Setup Action Request Recieved: " + action_str)
  success = False
  self.rbx_set_cmd_timeout_pub.publish(timeout_sec)
  time.sleep(0.1)
  action_ind = -1
  for ind, action in enumerate(self.rbx_cap_setup_actions):
    if action == action_str:
      action_ind = ind
  if action_ind == -1:
    nepi_sdk.log_msg_info("NEPI_RBX: No matching action found")
  else:
    nepi_sdk.log_msg_info("NEPI_RBX: Waiting for ready state for takeoff")
    ready = wait_for_rbx_status_ready(self,timeout_sec)
    if ready:
      nepi_sdk.log_msg_info("NEPI_RBX: Sending takeoff command")
...
def set_rbx_image_topic(self,image_topic):
  nepi_sdk.log_msg_info("NEPI_RBX: *******************************")  
  nepi_sdk.log_msg_info("NEPI_RBX: Set Image Topic Request Recieved: ")
  nepi_sdk.log_msg_info(image_topic)
  success = False
...
def set_rbx_process_name(self,process_name):
  nepi_sdk.log_msg_info("NEPI_RBX: *******************************")  
  nepi_sdk.log_msg_info("NEPI_RBX: Set Process Name Request Recieved: ")
  nepi_sdk.log_msg_info(process_name)
  success = False
...
def go_rbx_action(self,action_str,timeout_sec = 10):
  nepi_sdk.log_msg_info("NEPI_RBX: *******************************")  
  nepi_sdk.log_msg_info("NEPI_RBX: Goto Action Request Recieved: " + action_str)
  success = False
  self.rbx_set_cmd_timeout_pub.publish(timeout_sec)
  time.sleep(0.1)
  action_ind = -1
...
def go_rbx_home(self,timeout_sec = 10):
  nepi_sdk.log_msg_info("NEPI_RBX: *******************************")  
  nepi_sdk.log_msg_info("NEPI_RBX: Go Home Request Recieved: ")
  success = False
  self.rbx_set_cmd_timeout_pub.publish(timeout_sec)
  time.sleep(0.1)
  ready = wait_for_rbx_status_ready(self,timeout_sec)
...
def go_rbx_stop(self,timeout_sec = 10):
  nepi_sdk.log_msg_info("NEPI_RBX: *******************************")  
  nepi_sdk.log_msg_info("NEPI_RBX: Go Stop Request Recieved: ")
  self.rbx_set_cmd_timeout_pub.publish(timeout_sec)
  time.sleep(0.1)
...
def goto_rbx_pose(self,goto_data,timeout_sec = 10):
  ...
  if len(goto_data) == 3:
    ready = wait_for_rbx_status_ready(self,timeout_sec)
    if ready:
      nepi_sdk.log_msg_info("NEPI_RBX: Starting goto Attitude NED Process")
...
def goto_rbx_location(self,goto_data,timeout_sec = 10):
  ...
  if len(goto_data) == 4:
    ready = wait_for_rbx_status_ready(self,timeout_sec)
    if ready:
      nepi_sdk.log_msg_info("NEPI_RBX: Starting goto Location Global Process")
...
def goto_rbx_position(self,goto_data,timeout_sec = 10):
  ...
  if len(goto_data) == 4:
    ready = wait_for_rbx_status_ready(self,timeout_sec)
    if ready:
      nepi_sdk.log_msg_info("NEPI_RBX: Starting goto Position Body Process")
...
def wait_for_rbx_status_ready(self,timeout_sec = 10):
  nepi_sdk.log_msg_info("NEPI_RBX: Waiting for status ready")
  self.rbx_set_cmd_timeout_pub.publish(timeout_sec)
  time.sleep(0.1)
  ...
  timeout_timer = 0
  sleep_time_sec = 0.1
  while (counter < count_goal) and timeout_timer < timeout_sec and (not nepi_sdk.is_shutdown()):
    if self.rbx_status.ready is True:
      counter = counter + 1
      #nepi_sdk.log_msg_info("NEPI_RBX: Status ready counter updated to: " + str(self.rbx_status.ready))
    else:
      counter = 0
      #nepi_sdk.log_msg_info("NEPI_RBX: Status ready counter reset")
    time.sleep(sleep_time_sec)
    timeout_timer += sleep_time_sec
  if timeout_timer > timeout_sec:
    nepi_sdk.log_msg_info("NEPI_RBX: Aborted Wait for Ready due to timeout" + str(timeout_timer))
  else:
    nepi_sdk.log_msg_info("NEPI_RBX: Got status ready True")
  return self.rbx_status.ready 

### Function to wait for goto control process to complete
def wait_for_rbx_status_busy(self,timeout_sec = 10):
  nepi_sdk.log_msg_info("NEPI_RBX: Waiting for status busy")
  self.rbx_set_cmd_timeout_pub.publish(timeout_sec)
  time.sleep(0.1)
  count_goal = 3 # fix for strange ready glitch
  counter = 0
  ...
  while (counter < count_goal) and timeout_timer < timeout_sec and (not nepi_sdk.is_shutdown()):
    if self.rbx_status.ready is False:
      counter = counter + 1
      #nepi_sdk.log_msg_info("NEPI_RBX: Status busy counter updated to: " + str(self.rbx_status.ready))
    else:
      counter = 0
      #nepi_sdk.log_msg_info("NEPI_RBX: Status busy counter reset")
    time.sleep(sleep_time_sec)
    timeout_timer += sleep_time_sec
  if timeout_timer > timeout_sec:
    nepi_sdk.log_msg_info("NEPI_RBX: Aborted Wait for Busy due to timeout: " + str(timeout_timer))
  else:
    nepi_sdk.log_msg_info("NEPI_RBX: Got status busy True")
