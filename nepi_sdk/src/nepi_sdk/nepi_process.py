...
def update_processes_dict(processes_dict, process_name, process_dict):
    try:
        ...
        update_dict['results_display_dict'] = nepi_data.create_data_dict(process_dict.get('results_display_dict', dict()))
...
def get_process_dicts(processes_dict, process_name):
    ...
    if process_name in processes_dict.keys():
        ...
        try:
            results_display_dict = processes_dict[process_name].get('results_display_dict', dict())
        except Exception as e:
            logger.log_warn("Failed to get dict from proccesses dict " + str(e)) 

        try:
             states_dict = processes_dict[process_name].get('results_display_dict', dict())
        except Exception as e:
            logger.log_warn("Failed to get dict from proccesses dict " + str(e)) 

        
    return data_dict,controls_dict,results_display_dict,states_dict
...
def convert_results_pub_dict2msg( msg, msg_type, results_pub_dict):
    ...
    if msg is not None and msg_type is not None and results_pub_dict is not None:            
            results_display_dict = nepi_sdk.convert_msg2dict(msg())

            for result_name in results_display_dict.keys():
                if result_name in results_pub_dict.keys():
                    results_display_dict[result_name] = results_pub_dict[result_name]
            results_msg = nepi_sdk.convert_dict2msg(msg_type, results_display_dict)
    return results_msg
