...
SDK_FOLDER=$(cd -P "$(dirname -- "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)
SDK_NAME=$(basename "$SDK_FOLDER")

###############################################
# Deploy Nepi SDK
###############################################
SOURCE_PATH=$SDK_FOLDER
DEST_PATH="/mnt/nepi_storage/nepi_src/nepi_engine_ws/src/nepi_engine/nepi_sdk"
...
echo ""
echo "Syncing ${SDK_NAME} from ${SOURCE_PATH} to NEPI Build Repo at:" 
...
echo ""
echo "Syncing SDK FOLDER ${SDK_NAME} from ${SOURCE_PATH} to NEPI Live Folders at:" 
echo "Destination Path ${DEST_PATH}"
echo ""
